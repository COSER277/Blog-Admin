<template>
  <div class="friendList">
    ...
    <el-table :data="Data" style="width: 100%">
      <el-table-column label="友链ID" width="120">
      ...
      <el-table-column label="友链头像" width="120">
        <template slot-scope="scope">
          <img :src="scope.row.avatar" style="border-radius:50%;" width="40" height="40" />
        </template>
      </el-table-column>
      <el-table-column label="友人名称" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column label="博客邮箱" width="180">
        <template slot-scope="scope">
          <el-tag effect="plain" type="danger">{{ scope.row.email }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="友链状态" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column fixed="right" label="操作" width="200">
        ...
        <template slot-scope="scope">
          <el-button
            @click="$router.push(`/friends/edit/${scope.row._id}`)"
            type="text"
            size="small"
          >编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      const res = await this.$http.get("/common/friend/list");
      if (res.data.code == 0) {
        this.$message({
          type: "success",
          message: "获取列表数据成功!"
