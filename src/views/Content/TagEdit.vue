  ...
  <div class="categoryCreate">
    ...
    <el-form label-width="200px" @submit.native.prevent="save">
      ...
      <el-form-item label="标签图标">
        ...
        <el-input v-model="form.icon" v-if="upload_type"></el-input>
      </el-form-item>
      <el-form-item label="该属分类" label-width="200px">
        <el-select
...
export default {
  ...
  methods: {
    async fetch() {
      if (this.id) {
        const res = await this.$http.post(`/common/tag/${this.id}`);
        if (res.data.code == -1) {
          this.$message({
            type: "warning",
    ...
    async save() {
      if (this.id) {
        const res = await this.$http.put("/common/tag/update", this.form);
        if (res.data.code == 0) {
          this.$message({
            type: "success",
            message: "修改成功!"
        ...
        const res = await this.$http.post("/common/tag/create", this.form);
        if (res.data.code == -1) {
          this.$message({
            type: "warning",
            message: res.data.msg
