...
export default {
  ...
  methods: {
    async fetch() {
      if (this.id) {
        const res = await this.$http.post(`/common/banner/${this.id}`);
        if (res.data.code == -1) {
          this.$message({
            type: "warning",
    ...
    async save() {
      if (this.id) {
        const res = await this.$http.put("/common/banner/update", this.form);
        if (res.data.code == 0) {
          this.$message({
            type: "success",
            message: "修改成功!"
        ...
        const res = await this.$http.post("/common/banner/create", this.form);
        if (res.data.code == -1) {
          this.$message({
            type: "warning",
            message: res.data.msg
