<template>
  <div class="friendList">
    ...
    <el-table :data="Data" style="width: 100%">
      <el-table-column label="码库ID" width="120">
      ...
      <el-table-column fixed="right" label="操作" width="200">
        ...
        <template slot-scope="scope">
          <el-button
            @click="$router.push(`/codes/edit/${scope.row._id}`)"
            type="text"
            size="small"
          >编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      const res = await this.$http.get("/common/code/list");
      if (res.data.code == 0) {
        this.$message({
          type: "success",
          message: "获取列表数据成功!"
    ...
    remove(row) {
      this.$confirm(
        ...
        .then(async () => {
          const res = await this.$http.delete(`/common/code/remove/${row._id}`);
          if (res.data.code == 0) {
            this.$message({
              type: "success",
              message: "删除成功!"
